...
if(strpos($_SERVER['HTTP_HOST'], 'www.') !== false){
	...
	$href = str_replace('www.', '', $_SERVER['HTTP_HOST']).$_SERVER['REQUEST_URI'];
	
	header('HTTP/1.1 301 Moved Permanently');
	header("Location: http://".$href."", true, 301);
	exit(0);
}
...
// Язык
	...
	if($_SERVER['REQUEST_URI'] != '/'){
		...
		if(isset($tmp[1]) AND isset($languages[$tmp[1]])){
			$code = $languages[$tmp[1]]['code'];
		}
		
		//Если это Русский - он дефолтный
		if($code == 'ru' AND strpos($_SERVER['REQUEST_URI'],'/'.$code_deff.'/') !== false){
			
			$url = str_replace('/'.$code_deff.'/','',$_SERVER['REQUEST_URI']);
			
			$session->data['language'] = $code_deff;
			setcookie('language', $code_deff, time() + 60 * 60 * 24 * 30, '/', $request->server['HTTP_HOST']);
...
if (!isset($session->data['language']) || $session->data['language'] != $code || $code_deff == $code) {
	$session->data['language'] = $code;
}

if (!isset($request->cookie['language']) || $request->cookie['language'] != $code || $code_deff == $code) {
