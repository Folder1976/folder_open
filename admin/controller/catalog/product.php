...
class ControllerCatalogProduct extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			if (!empty($this->request->get['li_show'])) {
                $this->response->redirect( str_replace( HTTP_SERVER, HTTP_CATALOG, $this->url->link('product/product', 'product_id=' . $this->request->get['product_id'] ) ) );
                exit;
            }
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_category'])) {
			$filter_category = explode(',',trim($this->request->get['filter_category'],','));
		} else {
		...
		//======================================================================
		if (!empty($_SESSION['token']) and empty($this->request->get['li_sub'])) {
			$q = $this->db->query("SHOW TABLES LIKE '".DB_PREFIX."li_entities'")->row;
			if ($q) $q = $this->db->query("SELECT * FROM ".DB_PREFIX."li_entities WHERE platform_id=".$product_id)->row;
			if ($q) {
				$qq = $this->db->query("SELECT * FROM ".DB_PREFIX."li_ins WHERE ins_id=".$q['ins_id'])->row;
				$ourl = $q['ourl'];
			...
			$m .= '<div class="li_hint"><span style="color: orange; font-weight: bold">Notice:</span> this panel is visible only to the administrator of this shop</div>';
			$m .= '<div style="display: inline-block;position: absolute;white-space:no-wrap;overflow:hidden;float:right;">';
			$m .= '<a rel="nofollow" class="li_logo" style="font-size:14px; font-weight: bold; color: white; text-decoration: none" target="_blank" href="'.HTTP_SERVER.'admin/liveimport">';
			if ($q)
			...
			$m .= '<img src="admin/liveimport/lib/static/icon.png" style="vertical-align:middle;"> <font color="#8de43c">Live</font>Import</a>';

			if ($q) {
			 if ($qq) $m .= '&nbsp;&nbsp; <font color="#AAA">Task:</font> <a  rel="nofollow" style="font-size:14px; color: #7db4fb" target="_blank" href="'. HTTP_SERVER .'admin/liveimport/index.php?route=ins&id='.$qq['ins_id'].'&donor_id='.$qq['donor_id'] .'">'. $qq['title'] . '</a>';
			 $m .= '&nbsp;&nbsp;<font color="#AAA">Product source:</font> ';
			 if (strpos($ourl, "//")) $m .= '<a rel="nofollow noreferer" style="font-size:14px; color: #7db4fb" target="_blank" href="' . $ourl .'">' . $ourl . '</a>';
			...
			$m .= '<a  rel="nofollow" class="li_button" href="'.HTTPS_SERVER.'admin/index.php?route=catalog/product/edit&token='.$this->session->data['token'].'&product_id=' . $product_id.'">Edit Product</a>';
			...
			$m .= '<a  rel="nofollow" class="li_button" style="background: #222; color: #CCC !important" href="'.HTTP_SERVER.'admin/liveimport/index.php?op=ajax&command=delete_product&id='.$product_id.'">Delete Product</a>';

			$m .= "</div></div>";
			//$m .= '<div id="mx_more" style="display:none"><font color="grey">ID товара:</font> '.$q['url'].'</div>';

			$data['description'] = $m . $data['description'];
		...
		$data['category_tree'] = $this->model_catalog_product->getCategoryTree();
	...
	protected function getForm() {
		...
		$data['entry_description_detail'] = $this->language->get('entry_description_detail');
		...
		if (isset($this->request->post['original_code'])) {
			...
			$data['original_code'] = $product_info['original_code'];
		} else {
			$data['original_code'] = '';
		}
		...
		// Relationships
		$data['main_relationships'] = isset($this->request->get['product_id'])
			? $this->model_catalog_product->getProductToCategoryMainRelationships($this->request->get['product_id'])
			: null;
		...
		$data['entry_alternative_size'] = $this->language->get('entry_alternative_size');
		
		foreach($option_sizes as $row){
			$data['entry_'.$row] = $this->language->get('entry_'.$row);
			$data['option_entry']['entry_'.$row] = $this->language->get('entry_'.$row);
		}
		
		$data['option_sizes'] = $option_sizes;
	
		
		//$this->model_catalog_product->getOptionValueParams();
			...
			if (isset($product_option['product_option_value'])) {
				foreach ($product_option['product_option_value'] as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'params' => 				$this->model_catalog_option->getOptionValueParams($product_option_value['product_option_value_id']),
