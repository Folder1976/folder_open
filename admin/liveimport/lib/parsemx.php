...
// http_get function was renamed to http due to conflict with some PECL module
...
function script_check_duplicate($name = false, $timeout_mins = 5 ) {
    ...
    if ($pid)
        if (function_exists('posix_getsid') and !posix_getsid($pid)) $pid = false;
    if ($pid and $timeout) {
...
function remove_hash_cache($key) {
    save_hash_cache($key, "==delete==");
}
...
function clear_hash_cache() {
    ...
    $folders = glob($hash_cache_folder."/*");
    if (!$folders) return;
    foreach ($folders as $folder) {
        $ofiles = glob($folder . "/c*");
        if ($ofiles)
            foreach ($ofiles as $ofile) @unlink($ofile);
        rmdir($folder);
    }
}
...
function http($url)
{
    ...
    if (g('http_cache')) {
        ...
        if ($data) {
            ...
            xlog("Fetching <a rel='nofollow noreferrer' href='$url' target='_blank'>$url</a> (cache)");
    ...
    while ($n--) {
        ...
        if ($http_html or ($http_code >= 400 && $http_code<500)) break;
    }
    $http_ohtml = $http_html;
    if (!$http_html || $http_code<200 || $http_code>399) { re(false); return false; }
    if (g('http_cache') && $http_code==200) {
...
function http_ajax($url, $data = false) {
    ...
    if ($http_html[0] == "{" || $http_html[0] == "[")
        return re(json_decode($http_html, true));
...
function http_get_file($url, $save_path= '.', $access_path = false)
{
    ...
    while (is_numeric(substr($name,-1))) $name = substr($name,0,-1);
    if (!$name or !$ext) {
        $l = parse_url($url);
        $name = stristr($l['path'], "/");
    ...
    $n = 3;
    while (!file_exists($file) and $n--) {
        http_curl($url, '', $file);
        if ($http_code >= 400 && $http_code<500) break;
        if ($http_code == 304) break;
    }

    if (!file_exists($file)) { re(false); return false; }

    global $mx_curl_save_header;
    if (!$content_filename and $mx_curl_save_header) {
        $content_filename = inside("filename=", "", $mx_curl_save_header);
        if ($content_filename) {
            $content_filename = if_inside("", ";", $content_filename);
            $content_filename = if_inside("", "\n", $content_filename);
            $content_filename = trim($content_filename);
        }
    }


    if (g('http_cache'))
        save_hash_cache($url,$http_content_type.hash_cache_delimeter.file_get_contents($file).hash_cache_delimeter.$content_filename);

    $content_filename = substr($content_filename, 0, strrpos($content_filename, '.'));
    if ($content_filename) $name = $content_filename;

    if (g('http_content_type')) {
...
// Stored file should be without extension, and will be completed with extension after call
function http_if_file_changed(&$stored_file, $file_url, $skip_update_check = false) {

    if (substr($stored_file,-1)=='/') $stored_file .= crc32($file_url);
    $info_file = $stored_file.'.info';
    $new_file = false;
    $changed = false;
    ...
    if (file_exists($info_file)) {
        ...
        if ($skip_update_check and isset($info['ext']) and file_exists($stored_file.'.'.$info['ext'])) {
            $stored_file = $stored_file.'.'.$info['ext'];
            return re(false);
        }

        if (strpos($file_url, '//')!==false) {

            global $http_headers, $http_cache, $http_curl_timeout;
            $save_headers = $http_headers; $save_cache = $http_cache;

            if (file_exists($stored_file.'.'.$info['ext']))
                $http_headers = array('If-Modified-Since' => gmdate("D, d M Y H:i:s \G\M\T", $info['last_modified']));
            else $http_headers = false;
            $http_cache = false;
            $http_curl_timeout = 5*60;
            set_time_limit(5*60);
            //xlog($http_headers);
            $new_file = http_get_file($file_url, $stored_file);
            $http_headers = $save_headers; $http_cache = $save_cache;

            if ($new_file) {
                $new_hash = hash_file('crc32', $new_file);
                if ($new_hash != $info['crc32']) {
                    $info['crc32'] = $new_hash;
                    $changed = true;
                } else {
                    unlink($new_file);
            ...
            if (isset($info['ext'])) $stored_file = $stored_file.'.'.$info['ext'];
            ...
            if (filemtime($file_url) > $info['last_modified']) {
                $new_hash = hash_file('crc32', $file_url);
                if ($new_hash != $info['crc32']) {
                    $info['crc32'] = $new_hash;
                    $changed = true;
                }
        ...
        if (strpos($file_url, '//')!==false) {
            ...
            $info['crc32'] = hash_file('crc32', $new_file);
            ...
            $stored_file = $file_url;
            if (!file_exists($file_url)) return re(false);
            $info['crc32'] = hash_file('crc32', $file_url);

        }
    }
    if ($changed) $info['last_modified'] = time();

    if ($new_file) {
        @unlink($stored_file);
        $info['ext'] = rev_inside('.', '', $new_file);
        if ($new_file != $stored_file) rename($new_file, $stored_file);
    }

    file_put_contents($info_file, serialize($info));
    return re($changed);
}
...
function url_path_encode($url) { // Prepare URL for CURL fetch
    $domain = domain($url);
    if (strlen8($domain) != strlen($domain)) {
        $domain2 = idn_to_ascii($domain);
        $url = str_replace($domain, $domain2, $url);
    }
...
function http_curl($url, $post = false, $file = false)
{
    ...
    // Proxy code
    global $http_proxy, $http_proxies_file, $http_use_proxies;
    if ($http_use_proxies and !$http_proxy)
    ...
    xlog("Fetching <a rel='nofollow noreferrer' href='$url' target='_blank'>$url</a>...");

    $ch = curl_init(url_path_encode($url));
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    if (($http_cookies!==false) and ($http_cookies!==NULL)) curl_setopt($ch, CURLOPT_COOKIE, $http_cookies);
        ...
        $cookie_file = g('http_cookies_file');
    ...
    global $http_url_base;
    if (!$http_url_base) $http_url_base = $url;
    ...
    if (@!filesize($file) || $http_code>399) @unlink($file);
    ...
    if ($http_html) {
        ...
        if (strtoupper($encoding)=="UTF8") $encoding = "UTF-8";
        if ($encoding != "BINARY") {
...
function curl_redir_exec($curl_handler, $file_name = false, $file_handler = false)
{
    ...
    if ($http_code == 301 || $http_code == 302)
    {
        ...
        xlog("Redirected to <a rel='nofollow noreferrer' target='_blank' href='$new_url'>$new_url</a>...");
...
function insides($start, $end="", $source = false)
{
    ...
    if (is_array($source)) {
        foreach ($source as $line)
            if ($start and $end) $r = array_merge($r, insides($start,$end,$line));
            else {
        ...
        if (strpos($url, '://') === false)
            ...
            elseif ($url[0] == '?') $url = $http_url_base . $url;
                ...
                while (substr($url, 0, 3) == '../') {
                    $p = strrpos(substr($base, 0, -1), '/');
                    if ($p and ($base[$p-1] != '/')) $base = substr($base,0, $p+1);
                    $url = substr($url, 3);
                }
                $url = $base . $url;
            }
...
function re($return = '[no return value]') {
    ...
    if (!$debug_nesting_level or !ini_get('display_errors')) return $return;
...
function mxReportFileRepresentation($file, $line) {
    $start_dir = g('mx_save_start_dir').'/';
    $short_file = $file;
    if (substr($short_file, 0, strlen($start_dir)) == $start_dir)
        $short_file = substr($short_file, strlen($start_dir));
    $short_file = "<strong>$short_file</strong>:$line";
    if (g('mx_file_viewer'))
        $file = "<a target='_blank' href='".g('mx_file_viewer')."$file:$line'>$short_file</a>";
    else $file = $short_file;
    return $file;
}
...
function mxErrorHandler($errno, $errstr, $errfile, $errline)
{
    ...
    $file = mxReportFileRepresentation($errfile, $errline);

    switch ($errno) {

        case E_USER_WARNING:
            xwarn("$errstr at $file");
            break;

        default:
            xnotice("$errstr at $file");
            break;
    }
...
function mxshutdown() {
    ...
    if($error !== NULL)
        if (($error['type']!=2) and ($error['type']!=8) and ($error['line'])) {
            ini_set('display_errors',1);
            if (!strpos($error['message'],'deprecated'))  xwarn($error['message'] . " at ".mxReportFileRepresentation($error['file'], $error['line']));
        }
}
...
function close_tags($http_html) {
    ...
    foreach ($tags as $i=>&$t) {
        if (strpos($t, '<') !== false) {
            $http_html = str_replace($t, '', $http_html);
            unset($tags[$i]);
            continue;
        }
