...
static function start_once() {
  if (get_class(jsrt::$global)=="js_object") return; // Lets reset the processor MX
  jsrt::start();
}
...
static function call($method, $args) {
  ...
  if ($obj instanceof js_function) { // XXX there could be other "callable" objects. maybe.
    ...
    if (g('test')) xlog("JS: Call to undefined function '".$method->propName."'.");//MXI
    return js_str("");
  }
}
...
*/
    ...
    //MXI
    jsrt::define_variable($id);
    $obj = jsrt::id($id);
    jsrt::expr_assign($obj,jsrt::literal_object());
    return $obj;
    //MXE
  }
  return jsrt::$idcache[$id];
...
class js_val {
  ...
  function getValue() {
    ...
    if (g('test')) xlog("##useless getValue##");
    return $this;
  }
  function putValue($w) {
...
class js_ref_null extends js_ref {
  ...
  function getValue() {
      if (g('test')) xlog("JS: Read of undefined property '".$this->propName."'.");//MXI
