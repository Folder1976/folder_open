...
<h1>История изменения цен ЗАКУП</h1>
...
<?php
	...
	$products_no_sort = array();
			...
			$sql = 'SELECT PRP.date As rep_date,
					DATE_FORMAT( PRP.date , "%Y-%m-%d" ) AS `date_no_time`,
					PRP.zakup As rep_zakup,
					P.code AS product_code,
					P.*,
					U.*
				FROM '.DB_PREFIX.'product_rezakup PRP
...
<table class="find_table" style="background-color:#ebf4fb">
	<tr>
		...
		<a href="/backend/report_price/print.price.php?key=zakup&product_ids=<?php echo implode(',', $print_product_ids); ?>" id="print_operation" class="print_operation">
			...
			<?php foreach($products as $rows){ 
				...
				<?php $count = 1; foreach($rows as $row){ ?>
					...
					<tr>
						...
						<td style="text-align: right;"><?php echo number_format($row['rep_zakup'],2,'.',' '); ?></td>
