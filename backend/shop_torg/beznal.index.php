...
$List = $Operation->getBeznal($filter_data);
...
<?php foreach($List as $index => $rows){
  ...
  <?php //echo '<pre>'; print_r(var_dump( $row  )); ?>
	...
	<tr class="link_table_row" id="<?php echo $index; ?>"	>
		...
		<?php } ?>
		<td class="right">xxxx xxxx xxxx <b><?php echo $row['cart']; ?></b></td>
		<td class="center"><input type="checkbox" class="status"
		
				data-operation_id="<?php echo $row['operation_id']; ?>"
				data-sub_operation_id="<?php echo $row['sub_operation_id']; ?>"
		
				<?php if($row['status']) echo ' checked'; ?>></td>
	    <td class="mixed"><?php echo $UsersList[$row['user_id']]['lastname'] . ' ' . $UsersList[$row['user_id']]['firstname']; ?></td>
	    <td class="mixed"><?php
			if($row['validator_user_id'] > 0){
				echo $UsersList[$row['validator_user_id']]['lastname'] . ' ' . $UsersList[$row['validator_user_id']]['firstname'];
			}
			?></td>
		</tr>
