...
<table class="text">
	...
	<td class="mixed"><select id="country_shop_id" style="width:300px;">
                <?php foreach($CountryList as $index => $value){?>
                    <option value="<?php echo $index; ?>"><?php echo $value['name']; ?></option>
                <?php } ?>
...
<?php foreach($List as $index => $ex){ ?>
    <tr id="<?php echo $ex[$key];?>">
		...
		<td class="mixed"><select class="edit" id="country_shop_id<?php echo $ex[$key];?>" style="width:300px;">
				<option value="0">-</option>
                <?php foreach($CountryList as $index => $value){?>
                    <?php if($index == (int)$ex['country_shop_id']){ ?>
                        <option value="<?php echo $index; ?>" selected><?php echo $value['name']; ?></option>
                    <?php }else{ ?>
                        <option value="<?php echo $index; ?>"><?php echo $value['name']; ?></option>
                    <?php } ?>
...
<script>
    ...
    jQuery(document).on('change','.edit', function(){
        ...
        var country_shop_id = jQuery('#country_shop_id'+id).val();
        var enable_tmp = 0;
        ...
        jQuery.ajax({
            ...
            dataType: "text",
            data: "id="+id+"&description="+description+"&country_shop_id="+country_shop_id+"&country_id="+country_id+"&name="+name+"&enable="+enable_tmp+"&sort_order="+sort+"&key=edit_manufacturer",
            beforeSend: function(){
            },
    ...
    jQuery(document).on('click','.add', function(){
		...
		var country_shop_id = jQuery('#country_shop_id').val();
        var enable_tmp = 0;
        ...
        if (name != "") {
            jQuery.ajax({
                ...
                url: "/<?php echo TMP_DIR; ?>backend/ajax/ajax_guideuniversal.php",
                dataType: "text",
                data: "id="+id+"&description="+description+"&country_shop_id="+country_shop_id+"&country_id="+country_id+"&name="+name+"&enable="+enable_tmp+"&sort_order="+sort+"&key=add_manufacturer",
                beforeSend: function(){
                },
                success: function(msg){
                    console.log( msg );
